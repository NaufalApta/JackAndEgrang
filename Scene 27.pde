...
void draw() {
  ...
  // Gambar kakek dulu supaya Jack tampak di depan
  pushMatrix();
  scale(2.0);
  drawKakek(180, 165);
  popMatrix();

  // Gambar Jack setelah kakek agar tampak berjalan di depan
  float progress = map(currentFrame, 0, totalFrames, jackStartX, jackEndX);
  pushMatrix();
  scale(1.6);
  drawJack(progress, 255);
  popMatrix();
