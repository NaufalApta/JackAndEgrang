...
void draw() {
  ...
  float progress = constrain(map(currentFrame, 0, totalFrames, 0, 1), 0, 1);
  float kakekX = lerp(kakekStartX, kakekEndX, progress);
  pushMatrix();
  scale(2.0);
  drawKakek(kakekX, kakekY);
  popMatrix();
...
void drawJack(float x, float y) {
  ...
  stroke(0);
  strokeWeight(1.5);
  line(x - 10, y - 55, x - 3, y - 52);
  line(x + 3, y - 52, x + 10, y - 55);
  noFill();
  arc(x, y - 33, 14, 8, PI + QUARTER_PI, TWO_PI - QUARTER_PI);
  noStroke();
...
void drawKakek(float x, float y) {
  ...
  ellipse(x + 7, y - 46, 6, 6);
  fill(0);
  ellipse(x - 7, y - 46, 2, 2);
  ellipse(x + 7, y - 46, 2, 2);

  // Senyum
  noFill();
  stroke(0);
  arc(x, y - 36, 16, 8, 0, PI);
  noStroke();
