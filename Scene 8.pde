...
void drawJack(float x, float y) {
  ...
  // Mata
  fill(255);
  rect(x - 11, y - 50, 9, 6, 2);
  rect(x + 2, y - 50, 9, 6, 2);
  fill(0);
  ellipse(x - 7, y - 47, 3, 3);
  ellipse(x + 7, y - 47, 3, 3);

  // Mulut berbicara
  drawTalkingMouth(x, y - 34);
...
void drawTalkingMouth(float x, float y) {
  int phase = currentFrame % 24;
  if (phase < 6) {
    // Tutup
    stroke(0);
    strokeWeight(1);
