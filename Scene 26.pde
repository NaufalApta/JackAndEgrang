...
void draw() {
  float jackX = map(currentFrame, 0, totalFrames, 380, 170);
  ...
  // Kakek digambar duluan (agar Jack tampil di depan)
  pushMatrix();
  scale(2.0);
  drawKakek(180, 165);
  popMatrix();

  // Jack digambar setelah kakek, supaya posisinya "di depan"
  pushMatrix();
  scale(1.6);
  drawJack(jackX, 255);
  popMatrix();
